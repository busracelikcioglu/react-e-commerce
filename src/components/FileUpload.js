...
function FileUpload (props) {
    ...
    const onFilePicked = (e) => {
        ...
        if (props?.max_file_size_in_kb &&  file_size_kb > props?.max_file_size_in_kb) 
        {
            Swal.fire({
                icon: 'error',
                title: 'Oops...',
                text: 'İzin verilen maksimum fotoğraf boyutu = '+props?.max_file_size_in_kb+ ' kb',
        ...
        if (props?.allowed_extensions && !arrToLowerCase(props?.allowed_extensions).includes(file_type)) 
        {
            ...
            Swal.fire({
                icon: 'error',
                title: 'Oops...',
                text: 'İzin verilen dosya tipi = '+props?.allowed_extensions,
    ...
    return (
        <Fragment>
            <button className="btn btn-primary text-capitalize mr-2 mb-2"  onClick={(e) => onPickFile(e)}>{props?.button_title || 'Resim Yükle'}</button>
            
            {(props?.required && fileName?.length<=3 && !src ) ? <label className="label label-danger">Bu alan gereklidir</label> : ''}
            ...
            {/* new upload file */}
            {(props?.type=='image' && src && (props?.prev_src)) ? <img src={src}  style={{ maxHeight: "500px",  maxWidth: "300px" }} alt="" className="mt-2" /> : ''}
            
            {/* previous image */}
            {(props?.type=='image' && (props?.prev_src)  && !src) ?  <img src={props?.prev_src}  style={{ maxHeight: "500px",  maxWidth: "300px" }} alt="" className="mt-2" /> : ''}
